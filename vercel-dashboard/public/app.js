...
async function render() {
  ...
  const adminMatch = path.match(/^\/admin(?:\/(servers|users))?\/?$/);

  let session;
  try {
    session = await api("/api/session");
  } catch (err) {
    session = { user: null, banned: Boolean(err.data?.banned), reason: err.data?.reason || "" };
  }

  if (!session.user) {
    if (session.banned) renderBanned(session.reason);
    else renderLogin();
    return;
  }
  ...
  if (adminMatch) {
    if (!session.user.isAdmin) {
      ...
      renderAdmin(session.user, adminMatch[1] || "overview");
    }
  } else if (statusMatch) {
  ...
  app.innerHTML = `
    <div class="center-screen">
      <div class="login-card">
        ...
        ${new URLSearchParams(location.search).get("error") === "banned" ? `<div class="banner error" style="margin-bottom:16px">This Discord account has been banned from StatHive.</div>` : ""}
...
async function renderGuildPicker(user) {
  app.innerHTML = `
    <div class="topbar">
      ...
      <div class="topbar-right">${topbarRight(user)}</div>
    </div>
    <div class="page">
  ...
  app.innerHTML = `
    <div class="center-screen">
      <div class="login-card">
        ...
        <p>This account can no longer use StatHive.${reason ? ` Reason: ${escapeHtml(reason)}` : ""}</p>
        <a class="btn btn-secondary btn-block" href="/auth/logout">Log out</a>
      </div>
    </div>`;
}

// Profile chip, then the Admin button (admins only), then Log out
function topbarRight(user) {
  const shield = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l7 3v5c0 4.5-3 8.4-7 10-4-1.6-7-5.5-7-10V6l7-3z"/></svg>`;
  const crown = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M3 18h18l-1.5-9-4.5 4-3-6-3 6-4.5-4L3 18z"/></svg>`;
  return `${userChip(user)}${user.isOwner ? `<a class="btn btn-owner btn-sm" href="/owner" data-nav>${crown} Owner</a>` : ""}${user.isAdmin ? `<a class="btn btn-admin btn-sm" href="/admin" data-nav>${shield} Admin</a>` : ""}<a class="btn btn-ghost btn-sm" href="/auth/logout">Log out</a>`;
}

function adminBannerHtml(guild) {
  if (!guild || !guild.adminView) return "";
  return `<div class="admin-banner"><span>\u{1F6E1} Admin view \u2014 you\u2019re seeing this server the way its managers do.</span><a href="/admin/servers" data-nav>\u2190 Back to admin</a></div>`;
}
...
function drawChart(animate) {
  ...
  glowChart(wrap, chartRange === "24h" ? lastStats.series.slice(-24) : lastStats.series, chartRange, animate);
}

// opts: { unit: "players" (tooltip), colors: [start, mid, end], empty: "message" }
function glowChart(wrap, series, range, animate, opts = {}) {
  const [c1, c2, c3] = opts.colors || ["#5865f2", "#a855f7", "#22d3ee"];
  if (!series.some((p) => p.avg != null)) {
    wrap.innerHTML = `<div class="chart-empty">${escapeHtml(opts.empty || "Collecting data \u2014 the graph fills in as StatHive checks your server every 30 seconds.")}</div>`;
    return;
  }
  ...
  series.forEach((p, i) => {
    ...
    if (range === "7d" ? dt.getHours() === 0 : dt.getHours() % 4 === 0) {
      const lbl = range === "7d" ? dt.toLocaleDateString(undefined, { weekday: "short" }) : dt.toLocaleTimeString([], { hour: "numeric" });
    ...
    <svg class="glow-chart ${animate ? "anim" : ""}" viewBox="0 0 ${W} ${H}" role="img" aria-label="Players online over time">
      <defs>
        ...
        <linearGradient id="lg-area" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0" stop-color="${c2}" stop-opacity=".34" /><stop offset="1" stop-color="${c1}" stop-opacity="0" />
        </linearGradient>
  ...
  svg.addEventListener("pointermove", (e) => {
    ...
    tip.innerHTML = `${opts.unit ? `<b>${best.p.avg}</b> ${escapeHtml(opts.unit)}` : `<b>${best.p.avg}</b> avg \u00B7 <b>${best.p.max}</b> peak`}<span>${escapeHtml(new Date(best.p.t).toLocaleString([], { weekday: "short", hour: "numeric" }))}</span>`;
  });
...
document.addEventListener("click", (e) => {
  ...
  if (!href || href.startsWith("/auth/") || href.startsWith("/owner") || /^https?:\/\//i.test(href) || link.target === "_blank") return; // real navigations (login/logout/invite/owner dashboard)
  e.preventDefault();
  navigate(href);
});
